...
SegPtr uniqueAnchor(
    ...
    require(static_cast<bool>(block), "missing test Block");
    const auto [begin, end] = block->anchors.equal_range(key);
    require(begin != end, "missing test Block anchor");
    auto next = begin;
    ++next;
    require(next == end, "test expected one Block anchor occurrence");
    return begin->second;
}
...
void writeInput(const std::filesystem::path& path,
                ...
                size_t mismatch_length = 0,
                const std::string& chromosome = "chr1") {
    std::string sequence(10000, 'A');
    if (mismatch_start + mismatch_length > sequence.size()) {
        throw std::runtime_error("test mismatch interval exceeds FASTA");
    }
    std::fill_n(
        sequence.begin() + static_cast<std::ptrdiff_t>(mismatch_start),
        mismatch_length, 'T');
...
SegPtr addSegment(const BlockPtr& block,
    ...
    const SpeciesChrPair key{species, chromosome};
    require(block->anchors.find(key) == block->anchors.end(),
...
void populateZeroGapWindow(RaMesh::RaMeshMultiGenomeGraph& graph) {
    auto left = Block::createEmpty("simChimp", "chr1", 4);
    auto middle = Block::createEmpty("simChimp", "chr1", 3);
    auto right = Block::createEmpty("simChimp", "chr1", 4);
...
void populateSubsetZeroGapWindow(
    ...
    auto left = Block::createEmpty("simChimp", "chr1", 4);
    auto middle = Block::createEmpty("simChimp", "chr1", middle_species.size());
    auto right = Block::createEmpty("simChimp", "chr1", 4);
...
void populateFullParticipantGapWindow(
    RaMesh::RaMeshMultiGenomeGraph& graph,
    uint_t reference_gap = 5,
    uint_t query_gap = 6) {
    ...
    const auto chimp_left = addSegment(
        left, "simChimp", 0, 10, Strand::FORWARD);
    const auto chimp_right = addSegment(
        right, "simChimp", 10 + reference_gap, 10, Strand::FORWARD);
    const auto gorilla_left = addSegment(
        left, "simGorilla", 100, 10, Strand::FORWARD);
    const auto gorilla_right = addSegment(
        right, "simGorilla", 110 + query_gap, 10, Strand::FORWARD);

    linkPath(
        graph.species_graphs.at("simChimp").chr2end.at("chr1"),
...
void populateInteriorChain(
    ...
    for (const auto& participant_set : participants) {
        interiors.push_back(
            Block::createEmpty("simChimp", "chr1", participant_set.size()));
    }
    const std::vector<std::string> species_names = {
        "simChimp", "simGorilla", "simHuman", "simOrang"};
    for (size_t species_index = 0;
...
void populateConflictingOrdinaryHybridWindows(
    RaMesh::RaMeshMultiGenomeGraph& graph) {
    auto a = Block::createEmpty("simChimp", "chr1", 4);
    auto b1 = Block::createEmpty("simChimp", "chr1", 3);
    auto b2 = Block::createEmpty("simChimp", "chr1", 3);
    auto c = Block::createEmpty("simChimp", "chr1", 4);
    auto d1 = Block::createEmpty("simChimp", "chr1", 3);
    auto d2 = Block::createEmpty("simChimp", "chr1", 3);
    auto e = Block::createEmpty("simChimp", "chr1", 4);
    const std::vector<std::string> species_names = {
        "simChimp", "simGorilla", "simHuman", "simOrang"};
    for (size_t species_index = 0;
         species_index < species_names.size(); ++species_index) {
...
void populateFiveBlockWindows(
    ...
    auto a = Block::createEmpty("simChimp", "chr1", 4);
    auto b = Block::createEmpty("simChimp", "chr1", 3);
    auto c = Block::createEmpty("simChimp", "chr1", 4);
    auto d = Block::createEmpty("simChimp", "chr1", 3);
    auto e = Block::createEmpty("simChimp", "chr1", 4);
...
Cigar_t unbandedGlobalAlignmentForTest(
    ...
    if (reference.empty() || query.empty()) {
        ...
        if (!reference.empty()) {
            appendCigarOp(
                cigar, 'D', static_cast<uint32_t>(reference.size()));
        }
        if (!query.empty()) {
            appendCigarOp(
                cigar, 'I', static_cast<uint32_t>(query.size()));
        }
        return cigar;
    }
    ...
    ksw_extz2_sse(
        ...
        static_cast<int>(encoded_reference.size()),
        encoded_reference.data(),
        5, dna5_simd_mat,
        ksw2GapOpenPenalty(), ksw2GapExtendPenalty(),
        -1, -1, 0,
        KSW_EZ_RIGHT | KSW_EZ_GENERIC_SC,
        &result);
    Cigar_t cigar(
        result.cigar, result.cigar + result.n_cigar);
    free(result.cigar);
    return cigar;
}

void verifyCertifiedGlobalAlignmentMatchesFullKsw() {
    const std::vector<std::pair<std::string, std::string>>
        fixed_cases = {
            ...
            {"ACGTACGT", "ACGTTACGT"},
            {"ACGTTACGT", "ACGTACGT"},
            {std::string(200, 'A'), std::string(200, 'A')},
            {std::string(200, 'A'), std::string(200, 'T')}};
    for (const auto& [reference, query] : fixed_cases) {
        const Cigar_t certified =
            globalAlignKSW2(reference, query);
        const Cigar_t full =
            unbandedGlobalAlignmentForTest(reference, query);
        require(
            certified == full,
            "certified global alignment differs from full KSW2: " +
                cigarToString(certified) + " != " +
                cigarToString(full));
    }

    std::mt19937_64 random(20260814);
    static constexpr char bases[] = {'A', 'C', 'G', 'T', 'N'};
    for (size_t trial = 0; trial < 2000; ++trial) {
        const size_t reference_length = 1 + random() % 256;
        const size_t query_length = 1 + random() % 256;
        std::string reference(reference_length, 'A');
        std::string query(query_length, 'A');
        for (char& base : reference) {
            base = bases[random() % std::size(bases)];
        }
        if (trial % 2 == 0) {
            for (size_t index = 0;
                 index < std::min(reference_length, query_length);
                 ++index) {
                query[index] =
                    random() % 20 == 0
                        ? bases[random() % std::size(bases)]
                        : reference[index];
            }
            for (size_t index = reference_length;
                 index < query_length;
                 ++index) {
                query[index] = bases[random() % std::size(bases)];
            }
        } else {
            for (char& base : query) {
                base = bases[random() % std::size(bases)];
        ...
        require(
            certified == full,
            "random certified alignment differs from full KSW2 at trial " +
                std::to_string(trial) + ": " +
                cigarToString(certified) + " != " +
                cigarToString(full));
    }
...
void verifySimdGenericScoringMatchesScalar() {
    std::mt19937_64 random(20260814);
    static constexpr char bases[] = {'A', 'C', 'G', 'T', 'N'};
    for (size_t trial = 0; trial < 512; ++trial) {
        const size_t reference_length = 1 + random() % 257;
        const size_t query_length = 1 + random() % 257;
        std::string reference(reference_length, 'A');
        std::string query(query_length, 'A');
        for (char& base : reference) {
            base = bases[random() % std::size(bases)];
        ...
        const int zdrop = 1 + static_cast<int>(random() % 400);

        setenv("RAMAX_KSW_FORCE_SCALAR", "1", 1);
        const Cigar_t scalar_global =
            unbandedGlobalAlignmentForTest(reference, query);
        const Cigar_t scalar_extension =
            extendAlignKSW2(reference, query, zdrop);
        unsetenv("RAMAX_KSW_FORCE_SCALAR");

        const Cigar_t simd_global =
            unbandedGlobalAlignmentForTest(reference, query);
        const Cigar_t simd_extension =
            extendAlignKSW2(reference, query, zdrop);
        require(
            simd_global == scalar_global,
            "SIMD generic global scoring changed CIGAR at trial " +
                std::to_string(trial));
        require(
            simd_extension == scalar_extension,
            "SIMD generic extension scoring changed CIGAR at trial " +
                std::to_string(trial));
    }
}

void verifyShortBlockRepairKeepsCurrentReferenceKey(
    const std::filesystem::path& temp) {
    const std::vector<std::pair<std::string, std::string>> genomes = {
        {"simChimp", "simChimp.chr1"},
        {"simHuman", "simHuman.chr1"}};
    std::map<SpeciesName, SeqPro::SharedManagerVariant> managers;
    for (const auto& [species, chromosome] : genomes) {
        const auto fasta = temp / (species + "-short-block.fa");
        writeInput(fasta, 0, 0, chromosome);
        SeqPro::ManagerVariant manager =
            std::make_unique<SeqPro::SequenceManager>(fasta);
        managers[species] = std::make_shared<SeqPro::ManagerVariant>(
            std::move(manager));
    }

    RaMesh::RaMeshMultiGenomeGraph graph(managers);
    auto neighbor =
        Block::createEmpty("simChimp", "simChimp.chr1", genomes.size());
    auto short_block =
        Block::createEmpty("simChimp", "simChimp.chr1", genomes.size());
    for (const auto& [species, chromosome] : genomes) {
        const auto neighbor_segment = addSegment(
            neighbor, species, 0, 600, Strand::FORWARD, chromosome);
        const auto short_segment = addSegment(
            short_block, species, 600, 10, Strand::FORWARD, chromosome);
        linkPath(
            graph.species_graphs.at(species).chr2end.at(chromosome),
            {neighbor_segment, short_segment});
    }
    graph.blocks.push_back(neighbor);
    graph.blocks.push_back(short_block);

    RaMesh::ShortBlockRepair::Options options;
    options.enabled = true;
    options.maximum_short_length = 20;
    options.maximum_delete_length = 0;
    options.maximum_query_gap = 0;
    options.parallel_threads = 1;
    const auto result = RaMesh::ShortBlockRepair::repairFinalShortBlocks(
        graph, {"simHuman"}, managers, options);
    require(result.left_merged + result.right_merged == 1,
            "later-reference short Block was not merged");
    require(graph.blocks.size() == 1,
            "short Block repair did not replace both source Blocks");
    const auto merged = onlyActiveBlock(graph);
    require(merged->ref_species == "simHuman" &&
                merged->ref_chr == "simHuman.chr1",
            "short Block repair mixed the current reference species with "
            "the neighbor Block chromosome");
    uniqueAnchor(merged, {"simHuman", "simHuman.chr1"});
    require(
        merged->anchors.find({"simHuman", "simChimp.chr1"}) ==
            merged->anchors.end(),
        "short Block repair declared a nonexistent reference occurrence");
    require(graph.verifyGraphCorrectness(false),
            "later-reference short Block repair produced an invalid graph");

    merged->ref_chr = "simChimp.chr1";
    require(!graph.verifyGraphCorrectness(false),
            "graph verification accepted a missing declared reference");
    ...
    require(maf_rejected_invalid_reference,
            "MAF export silently selected a fallback reference row");
    merged->ref_chr = "simHuman.chr1";
    require(graph.verifyGraphCorrectness(false),
...
void releaseGraphForLeakCheck(
    RaMesh::RaMeshMultiGenomeGraph& graph) {
    std::vector<BlockPtr> live_blocks;
    live_blocks.reserve(graph.blocks.size());
    for (const auto& weak_block : graph.blocks) {
        if (auto block = weak_block.lock()) live_blocks.push_back(block);
    }

    for (auto& [species, genome] : graph.species_graphs) {
        (void)species;
        for (auto& [chromosome, end] : genome.chr2end) {
            (void)chromosome;
            std::vector<SegPtr> path;
            std::unordered_set<const Segment*> visited;
            SegPtr segment = end.head;
            while (segment && visited.emplace(segment.get()).second) {
            ...
            for (SegPtr& path_segment : path) {
                path_segment->primary_path.next.store(
                    nullptr, std::memory_order_release);
                path_segment->primary_path.prev.store(
                    nullptr, std::memory_order_release);
                path_segment->parent_block.reset();
                Cigar_t{}.swap(path_segment->cigar);
            }
            end.head.reset();
            end.tail.reset();
        }
    }
    for (BlockPtr& block : live_blocks) block->anchors.clear();
    graph.blocks.clear();
    graph.species_graphs.clear();
}

void verifyOverlapMergeRetiresSourceOwnership(
    const std::map<SpeciesName, SeqPro::SharedManagerVariant>& managers) {
    std::map<SpeciesName, SeqPro::SharedManagerVariant> test_managers = {
        {"simChimp", managers.at("simChimp")},
        {"simGorilla", managers.at("simGorilla")},
    ...
    auto left = Block::createEmpty("simChimp", "chr1", 2);
    auto right = Block::createEmpty("simChimp", "chr1", 2);
    auto chimp_left = addSegment(
        left, "simChimp", 100000, 100, Strand::FORWARD);
    auto chimp_right = addSegment(
        right, "simChimp", 100050, 100, Strand::FORWARD);
    auto gorilla = addSegment(
        left, "simGorilla", 200000, 100, Strand::FORWARD);
    auto human = addSegment(
        right, "simHuman", 300000, 100, Strand::REVERSE);
    ...
    const std::weak_ptr<Block> left_block_weak = left;
    const std::weak_ptr<Block> right_block_weak = right;
    const std::weak_ptr<Segment> chimp_left_weak = chimp_left;
    const std::weak_ptr<Segment> chimp_right_weak = chimp_right;
    const std::weak_ptr<Segment> gorilla_weak = gorilla;
    const std::weak_ptr<Segment> human_weak = human;
    ...
    require(!left_block_weak.expired() && !right_block_weak.expired(),
            "merge ownership fixture released source Blocks too early");
    graph.mergeMultipleGraphs("simChimp", 32);

    require(left_block_weak.expired() && right_block_weak.expired(),
            "overlap merge retained a retired source Block cycle");
    require(chimp_left_weak.expired() && chimp_right_weak.expired() &&
                gorilla_weak.expired() && human_weak.expired(),
            "overlap merge retained a retired source Segment");
    require(graph.blocks.size() == 3,
            "overlap merge weak Block pool contains retired entries");
    for (const auto& weak_block : graph.blocks) {
        require(!weak_block.expired(),
                "overlap merge left an expired weak Block entry");
    }

    const auto requireLiveSampling = [&](const SpeciesName& species) {
        auto& genome_end =
            graph.species_graphs.at(species).chr2end.at("chr1");
        std::unordered_set<const Segment*> live_segments;
        auto segment = genome_end.head;
        while (segment) {
            require(live_segments.emplace(segment.get()).second,
                    "sampling fixture contains a Segment cycle");
        ...
        require(segment == genome_end.tail,
                "sampling fixture path does not reach the tail");
        for (const auto& sampled : genome_end.sample_vec) {
            require(!sampled || live_segments.count(sampled.get()) == 1,
                    "sampling table retained a detached Segment");
        }
    };

    requireLiveSampling("simChimp");
    requireLiveSampling("simGorilla");
    requireLiveSampling("simHuman");
    require(graph.species_graphs.at("simChimp")
                    .chr2end.at("chr1")
                    .findSurrounding(100075)
                    ->start == 100050,
            "rebuilt reference sampling returned the wrong predecessor");
    require(graph.verifyGraphCorrectness(false),
            "ownership-retirement merge produced an invalid graph");
    require(graph.compactBlockPool() == 0,
            "compacting a clean merge Block pool removed live entries");
    releaseGraphForLeakCheck(graph);
}

void verifyOverlapMergeStressRetiresSources(
    const std::map<SpeciesName, SeqPro::SharedManagerVariant>& managers) {
    constexpr size_t kMergeCount = 100000;
    constexpr size_t kSourceBlockCount = kMergeCount + 1;
    ...
    std::vector<std::weak_ptr<Block>> source_blocks;
    std::vector<std::weak_ptr<Segment>> source_segments;
    source_blocks.reserve(kSourceBlockCount);
    source_segments.reserve(kSourceBlockCount);
    graph.blocks.reserve(kSourceBlockCount);

    SegPtr previous = genome_end.head;
    for (size_t index = 0; index < kSourceBlockCount; ++index) {
        auto block = Block::createEmpty("simChimp", "chr1", 1);
        auto segment = addSegment(
            block, "simChimp", 100000, 100, Strand::FORWARD);
        previous->primary_path.next.store(
            segment, std::memory_order_release);
        segment->primary_path.prev.store(
            previous, std::memory_order_release);
    ...
    genome_end.tail->primary_path.prev.store(
        previous, std::memory_order_release);
    genome_end.sample_vec.assign(1, genome_end.head);
    genome_end.setToSampling(previous);
    previous.reset();

    graph.mergeMultipleGraphs("simChimp", 32);

    for (const auto& source_block : source_blocks) {
        require(source_block.expired(),
                "stress merge retained an original source Block");
    }
    for (const auto& source_segment : source_segments) {
        require(source_segment.expired(),
                "stress merge retained an original source Segment");
    }
    require(graph.blocks.size() == 1,
            "stress merge did not compact the weak Block pool to one result");
    require(!graph.blocks.front().expired(),
            "stress merge final Block is not live");
    require(graph.verifyGraphCorrectness(false),
            "stress merge produced an invalid final graph");
    require(genome_end.findSurrounding(100050)->start == 100000,
            "stress merge rebuilt sampling with the wrong predecessor");
    require(graph.compactBlockPool() == 0,
            "stress merge left expired entries in the weak Block pool");
    releaseGraphForLeakCheck(graph);
}
...
int main() {
    ...
    try {
        const bool retirement_only =
            std::getenv("RAMAX_RUN_MERGE_RETIREMENT_ONLY") != nullptr;
        const bool stress_only =
            std::getenv("RAMAX_RUN_MERGE_STRESS_ONLY") != nullptr;
        require(!(retirement_only && stress_only),
                "select only one focused merge test mode");
        if (!retirement_only && !stress_only) {
            verifyCertifiedGlobalAlignmentMatchesFullKsw();
            verifySimdGenericScoringMatchesScalar();
            verifyShortBlockRepairKeepsCurrentReferenceKey(temp);
        }
        std::map<SpeciesName, SeqPro::SharedManagerVariant> managers;
        ...
        if (retirement_only) {
            verifyOverlapMergeRetiresSourceOwnership(managers);
            std::filesystem::remove_all(temp);
            return 0;
        }
        if (stress_only) {
            verifyOverlapMergeStressRetiresSources(managers);
            std::filesystem::remove_all(temp);
            return 0;
        }
        ...
        std::map<SpeciesName, SeqPro::SharedManagerVariant>
            pair_managers = {
                {"simChimp", managers.at("simChimp")},
                {"simGorilla", managers.at("simGorilla")}};
        RaMesh::RaMeshMultiGenomeGraph gap_bridge_graph(pair_managers);
        populateFullParticipantGapWindow(gap_bridge_graph);
        require(gap_bridge_graph.realignSingleMissingSpeciesWindows(
                    "simChimp", pair_managers, "/bin/false",
                    20, 2, 20) == 1,
                "full-participant positive-gap boundary was not bridged");
        require(gap_bridge_graph.blocks.size() == 1,
                "positive-gap bridge did not reduce two Blocks to one");
        const auto gap_bridge_block = onlyActiveBlock(gap_bridge_graph);
        const auto gap_bridge_chimp =
            uniqueAnchor(gap_bridge_block, {"simChimp", "chr1"});
        const auto gap_bridge_gorilla =
            uniqueAnchor(gap_bridge_block, {"simGorilla", "chr1"});
        require(gap_bridge_chimp->start == 0 &&
                    gap_bridge_chimp->length == 25 &&
                    countRefLength(gap_bridge_chimp->cigar) == 25 &&
                    countQryLength(gap_bridge_chimp->cigar) == 25,
                "positive-gap reference interval was not preserved");
        require(gap_bridge_gorilla->start == 100 &&
                    gap_bridge_gorilla->length == 26 &&
                    countRefLength(gap_bridge_gorilla->cigar) == 25 &&
                    countQryLength(gap_bridge_gorilla->cigar) == 26,
                "positive-gap query interval was not preserved");
        require(gap_bridge_graph.verifyGraphCorrectness(false),
                "positive-gap bridge graph verification failed");

        require(alignmentRowsPreferredToUnaligned("AAAA", "AAAA"),
                "identical alignment lost to the unaligned model");
        require(alignmentRowsPreferredToUnaligned("A", "T"),
                "anchored single substitution lost to the unaligned model");
        require(!alignmentRowsPreferredToUnaligned(
                    std::string(100, 'A'), std::string(100, 'T')),
                "long unrelated alignment beat the unaligned model");
        require(alignmentCigarPreferredToUnaligned(
                    "ACGT", "ACGT",
                    Cigar_t{cigarToInt('M', 4)}),
                "identical CIGAR lost to the unaligned model");
        require(!alignmentCigarPreferredToUnaligned(
                    std::string(100, 'A'), std::string(100, 'T'),
                    Cigar_t{cigarToInt('M', 100)}),
                "unrelated CIGAR beat the unaligned model");

        std::map<SpeciesName, SeqPro::SharedManagerVariant>
            mismatch_pair_managers;
        for (const std::string species : {"simChimp", "simGorilla"}) {
            const auto fasta =
                temp / (species + "-bridge-mismatch.fa");
            if (species == "simGorilla") {
                writeInput(fasta, 110, 100);
            } else {
                writeInput(fasta);
            }
            SeqPro::ManagerVariant manager =
                std::make_unique<SeqPro::SequenceManager>(fasta);
            mismatch_pair_managers[species] =
                std::make_shared<SeqPro::ManagerVariant>(
                    std::move(manager));
        }
        RaMesh::RaMeshMultiGenomeGraph mismatch_bridge_graph(
            mismatch_pair_managers);
        populateFullParticipantGapWindow(
            mismatch_bridge_graph, 100, 100);
        require(mismatch_bridge_graph.realignSingleMissingSpeciesWindows(
                    "simChimp", mismatch_pair_managers, "/bin/false",
                    200, 2, 200) == 1,
                "low-score full-participant boundary was not compressed");
        const auto mismatch_bridge_block =
            onlyActiveBlock(mismatch_bridge_graph);
        const auto mismatch_gorilla = uniqueAnchor(
            mismatch_bridge_block, {"simGorilla", "chr1"});
        require(cigarToString(mismatch_gorilla->cigar) ==
                    "10M100D100I10M",
                "low-score bridge was emitted as aligned homology");
        require(mismatch_bridge_graph.verifyGraphCorrectness(false),
                "low-score bridge graph verification failed");
        ...
        const auto orang = uniqueAnchor(merged, {"simOrang", "chr1"});
        ...
        const auto gorilla = uniqueAnchor(merged, {"simGorilla", "chr1"});
        ...
        require(cigarToString(uniqueAnchor(two_missing_block, {"simHuman", "chr1"})->cigar) == "10M20D10M" &&
                    cigarToString(uniqueAnchor(two_missing_block, {"simOrang", "chr1"})->cigar) == "10M20D10M",
        ...
        for (const std::string species :
            ...
            require(cigarToString(uniqueAnchor(three_missing_block, {species, "chr1"})->cigar) == "10M20D10M",
        ...
        uniqueAnchor(hybrid_right, {"simOrang", "chr1"})->start = 310;
        ...
        const auto hybrid_orang = uniqueAnchor(hybrid_block, {"simOrang", "chr1"});
        ...
        uniqueAnchor(serial_hybrid_right, {"simOrang", "chr1"})->start = 310;
        ...
        uniqueAnchor(insertion_right, {"simOrang", "chr1"})->start = 310;
        ...
        uniqueAnchor(reverse_empty_left, {"simGorilla", "chr1"})->start = 110;
        ...
        const auto reverse_empty_gorilla =
            uniqueAnchor(reverse_empty_block, {"simGorilla", "chr1"});
        ...
        uniqueAnchor(reference_only_right, {"simGorilla", "chr1"})->start =
            110;
        uniqueAnchor(reference_only_right, {"simHuman", "chr1"})->start =
            210;
        uniqueAnchor(reference_only_right, {"simOrang", "chr1"})->start =
            310;
        ...
        for (const std::string species :
             {"simGorilla", "simHuman", "simOrang"}) {
            require(cigarToString(uniqueAnchor(reference_only_block, {species, "chr1"})->cigar) == "10M30D10M",
        ...
        auto replacement = reverse_multiblock_graph.species_graphs
                               ...
                               ->primary_path.next.load(
                                   std::memory_order_acquire)
                               ->parent_block;
        const auto reverse_anchor = uniqueAnchor(replacement, {"simGorilla", "chr1"});
        ...
        uniqueAnchor(mismatch_right, {"simGorilla", "chr1"})->strand =
            Strand::FORWARD;
        ...
        uniqueAnchor(order_mismatch_right, {"simOrang", "chr1"})->start =
            100;
        ...
        require(counter_contents == "x",
                "deterministic rerun bypassed the external MSA cache");
