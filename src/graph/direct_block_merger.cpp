...
struct PreparedChain {
    ...
    size_t unaligned_bridge_paths = 0;
};
...
#ifdef RAMAX_REALIGNMENT_ORACLE_SINGLE_BATCH
constexpr size_t kZeroGapBatchMaximumWindows =
    std::numeric_limits<size_t>::max();
constexpr size_t kMinipoaBatchMaximumWindows =
    std::numeric_limits<size_t>::max();
#else
...
void maybeLogRealignmentProgress(
    const SpeciesName& reference_species,
    const char* phase,
    size_t generation,
    size_t processed,
    ...
    if (now - last_report < std::chrono::minutes(10)) return;
    last_report = now;
    const auto memory = readProcessMemorySnapshot();
    spdlog::info(
        "[species-mismatch-realign][progress] reference={} phase={} "
        "generation={} processed={} candidates={} pending={} committed={} "
        "batch_size={} elapsed_seconds={:.1f} vm_rss_kib={} vm_peak_kib={}",
        reference_species, phase, generation, processed, candidates,
        pending, committed, batch_size,
...
constexpr size_t kInProcessPairwiseMaximumLength = 512;
constexpr uint_t kFullParticipantBridgeMaximumSpan = 1000000;
...
bool anchorsContainSegment(
    const ChrHeadMap& anchors,
    const SpeciesChrPair& key,
    const SegPtr& segment) {
    const auto [begin, end] = anchors.equal_range(key);
    return std::any_of(begin, end, [&](const auto& entry) {
        return entry.second == segment;
    });
}

bool insertUniqueAnchor(
    const BlockPtr& block,
    const SpeciesChrPair& key,
    const SegPtr& segment) {
    if (!block || !segment || block->anchors.find(key) != block->anchors.end()) {
        return false;
    }
...
PreparedChain prepareChain(const CandidateChain& candidate,
    ...
    for (const auto& [key, unused] : keys) {
        ...
        if (!insertUniqueAnchor(
                prepared.merged_block, key, path.merged_segment)) {
            throw std::runtime_error(
                "Merged exact-contiguous block has duplicate participant");
        }
...
bool rebuildSamplingAndAuditAffectedGraph(
    ...
    for (const auto& chain : prepared) {
        ...
        for (const auto& path : chain.paths) {
            if (!path.merged_segment ||
                ...
                !anchorsContainSegment(
                    chain.merged_block->anchors,
                    path.key,
                    path.merged_segment) ||
    ...
    for (const auto& [key, snapshot] : affected) {
        ...
        while (current && !current->isTail()) {
            ...
            if (!anchorsContainSegment(parent->anchors, key, current) ||
...
struct MissingWindowCandidate {
    ...
    bool full_participant_bridge = false;
...
MissingWindowPlan makeMissingWindowPlan(
    ...
    plan.extended_bridge = extended_bridge;
...
MissingWindowReject buildMissingWindowCandidate(
    ...
    if (candidate.missing_species.empty() && candidate.adjacent_pair) {
        candidate.full_participant_bridge = true;
    }
    ...
    if (!zero_gap_phase) {
        ...
        const uint_t span_limit =
            candidate.full_participant_bridge
                ? kFullParticipantBridgeMaximumSpan
                : maximum_span;
        for (const auto& path : candidate.paths) {
            ...
            if (path.interval_length > span_limit) {
                return MissingWindowReject::INTERVAL_INVALID;
            }
        }
        if (!any_nonzero) {
...
std::optional<PreparedChain> prepareZeroGapDeletionWindow(
    ...
    for (const auto& context : candidate.paths) {
        ...
        path.merged_segment->right_extend =
            path.old_segments.back()->right_extend;
        if (!insertUniqueAnchor(
                prepared.merged_block, context.key, path.merged_segment)) {
            return reject_prepared();
        }
        prepared.paths.push_back(std::move(path));
    }
...
std::optional<PreparedChain> prepareMissingWindow(
    ...
    const bool replace_boundary_blocks =
    ...
    raw_sequences.reserve(candidate.paths.size());
    {
        ...
        for (const auto& path : candidate.paths) {
            if (replace_boundary_blocks && path.interval_length == 0) {
                continue;
            }
            std::string sequence;
    ...
    if (aligned_sequences.size() == 1 &&
        ...
        aligned_sequences.size() == 2 &&
        aligned_sequences.count(reference_species) == 1 &&
        std::all_of(
            aligned_sequences.begin(), aligned_sequences.end(),
            [](const auto& entry) {
                return entry.second.size() <=
                       kInProcessPairwiseMaximumLength;
            })) {
        const auto query_it = std::find_if(
            aligned_sequences.begin(), aligned_sequences.end(),
            [&](const auto& entry) {
                return entry.first != reference_species;
            });
        try {
            const auto cigar = globalAlignKSW2(
                aligned_sequences.at(reference_species), query_it->second);
            auto [aligned_reference, aligned_query] = buildAlignment(
                aligned_sequences.at(reference_species), query_it->second,
                cigar);
            if (aligned_reference.size() == aligned_query.size()) {
                aligned_sequences[reference_species] =
                    std::move(aligned_reference);
                aligned_sequences[query_it->first] =
                    std::move(aligned_query);
                msa_succeeded = true;
            }
        } catch (const std::exception& error) {
            spdlog::warn(
                "[species-mismatch-realign] in-process pairwise "
                "alignment failed: {}",
                error.what());
        }
    } else {
    ...
    if (replace_boundary_blocks) {
        ...
        for (const auto& context : candidate.paths) {
            ...
            if (context.interval_length == 0) {
                ...
                if (candidate.full_participant_bridge &&
                    !alignmentRowsPreferredToUnaligned(
                    ...
                    ++prepared.unaligned_bridge_paths;
                }
            }
...
size_t RaMeshMultiGenomeGraph::realignSingleMissingSpeciesWindows(
    ...
    const auto realignment_started = std::chrono::steady_clock::now();
    spdlog::info(
        "[species-mismatch-realign][phase] reference={} phase=start "
        "blocks={} species={} threads={} maximum_span={} "
        "zero_gap_span={}",
        reference_species, blocks.size(), seqpro_managers.size(),
        parallel_threads, maximum_span, zero_gap_maximum_span);
    const auto pool_index_started = std::chrono::steady_clock::now();
    AllocationPhaseScope pool_index_phase("pool-index");
    BlockPoolEditor pool_editor(blocks);
    spdlog::info(
        "[species-mismatch-realign][phase] reference={} phase=pool-index "
        "status=done blocks={} wall_seconds={:.3f}",
        reference_species, pool_editor.activeCount(),
        std::chrono::duration<double>(
            std::chrono::steady_clock::now() - pool_index_started).count());
    std::set<SpeciesChrPair> affected_paths;
    ...
    size_t zero_gap_reference_visits = 0;
    const auto zero_gap_started = std::chrono::steady_clock::now();
    auto zero_gap_last_report = zero_gap_started;
    spdlog::info(
        "[species-mismatch-realign][phase] reference={} phase=zero-gap "
        "status=start",
        reference_species);
    ...
    while (true) {
        ...
        const auto scan_zero_gap_start =
            [&](const ChrName& chromosome,
                ...
                ++zero_gap_reference_visits;
                if ((zero_gap_reference_visits & 65535U) == 0) {
                    maybeLogRealignmentProgress(
                        reference_species, "zero-gap-scan",
                        zero_gap_scan_count, zero_gap_reference_visits,
                        zero_gap_candidates.size(), 0,
                        zero_gap_replaced_windows, 0,
                        zero_gap_started, zero_gap_last_report);
                }
        ...
        std::vector<size_t> anchor_counts;
        footprints.reserve(zero_gap_candidates.size());
        anchor_counts.reserve(zero_gap_candidates.size());
        for (const auto& candidate : zero_gap_candidates) {
            auto reads = read_blocks(candidate);
            footprints.push_back({reads, std::move(reads)});
            anchor_counts.push_back(candidate.boundary_species_count);
        }
        std::vector<size_t> ordered(zero_gap_candidates.size());
        std::iota(ordered.begin(), ordered.end(), 0);
        Realignment::MissingWindowBatchScheduler scheduler(
            std::move(ordered), footprints, anchor_counts);

        while (!scheduler.empty()) {
            const std::vector<size_t> batch = scheduler.nextBatch(
                kZeroGapBatchMaximumWindows,
                kZeroGapBatchMaximumAnchors, true);
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(effective_threads)
            ...
            std::vector<MissingWindowCandidate> accepted_candidates;
            std::vector<PreparedChain> prepared;
            std::vector<bool> batch_succeeded(batch.size(), false);
            ...
            scheduler.completeBatch(batch, batch_succeeded);
            maybeLogRealignmentProgress(
                reference_species, "zero-gap-schedule",
                zero_gap_scan_count, zero_gap_reference_visits,
                zero_gap_candidates.size(),
                scheduler.pendingCandidates(),
                committed_this_iteration, batch.size(),
                zero_gap_started, zero_gap_last_report);
        }

        overlap_this_iteration = scheduler.droppedConflicts();
        zero_gap_rejections[static_cast<size_t>(
    ...
    spdlog::info(
        "[species-mismatch-realign][phase] reference={} phase=zero-gap "
        "status=done scans={} visited={} candidates={} committed={} "
        "wall_seconds={:.3f}",
        reference_species, zero_gap_scan_count,
        zero_gap_reference_visits, zero_gap_candidate_events,
        zero_gap_replaced_windows,
        std::chrono::duration<double>(
            std::chrono::steady_clock::now() - zero_gap_started).count());
    ...
    size_t bridge_committed_total = 0;
    size_t bridge_extended_scanned_total = 0;
    size_t bridge_extended_candidates_total = 0;
    size_t bridge_extended_committed_total = 0;
    uint64_t bridge_extended_bases_total = 0;
    size_t minipoa_invocations_total = 0;
    size_t reference_only_bypass_total = 0;
    size_t reference_empty_bypass_total = 0;
    size_t unaligned_bridge_paths_total = 0;
    ...
    size_t minipoa_reference_visits = 0;
    const auto minipoa_started = std::chrono::steady_clock::now();
    auto minipoa_last_report = minipoa_started;
    spdlog::info(
        "[species-mismatch-realign][phase] reference={} phase=minipoa "
        "status=start",
        reference_species);
    ...
    while (true) {
        ...
        const auto scan_minipoa_start =
            [&](const ChrName& chromosome,
                ...
                ++minipoa_reference_visits;
                if ((minipoa_reference_visits & 65535U) == 0) {
                    maybeLogRealignmentProgress(
                        reference_species, "minipoa-scan",
                        minipoa_rounds, minipoa_reference_visits,
                        candidate_slots.size(), 0,
                        ordinary_committed_total + hybrid_committed_total +
                            bridge_committed_total,
                        0, minipoa_started, minipoa_last_report);
                }
                const auto next_reference =
                    left_reference->primary_path.next.load(
                        std::memory_order_acquire);
                ...
                bool reference_span_exceeded = false;
                bool extended_bridge_scan = false;
                const uint64_t left_reference_end =
                    segmentEnd(left_reference);
                while (cursor && !cursor->isTail()) {
                    ...
                    const bool same_anchor_keys =
                        sameAnchorKeys(left_view, cursor_view);
                    const bool adjacent_same_signature =
                        interior_blocks.empty() && same_anchor_keys;
                    const uint64_t span_limit =
                        adjacent_same_signature
                            ? std::max<uint64_t>(
                                  maximum_span,
                                  kFullParticipantBridgeMaximumSpan)
                            : static_cast<uint64_t>(maximum_span);
                    if (static_cast<uint64_t>(cursor->start) >
                        left_reference_end + span_limit) {
                        reference_span_exceeded = true;
                        break;
                    }
                    if (same_anchor_keys) {
                        extended_bridge_scan =
                            adjacent_same_signature &&
                            static_cast<uint64_t>(cursor->start) >
                                left_reference_end +
                                    static_cast<uint64_t>(maximum_span);
                        if (extended_bridge_scan) {
                            ++bridge_extended_scanned_total;
                        }
                ...
                if (!right_reference) {
                    if (reference_span_exceeded) {
                        ++rejections[static_cast<size_t>(
                            MissingWindowReject::INTERVAL_INVALID)];
                    } else if (incompatible_boundary) {
                        ++rejections[static_cast<size_t>(
                            MissingWindowReject::
                                INCOMPATIBLE_BOUNDARY_SIGNATURE)];
                    }
                ...
                if (extended_bridge_scan) {
                    ...
                    ++bridge_extended_candidates_total;
                }
                if (candidate.full_participant_bridge) {
                    ++bridge_this_round;
                } else if (candidate.hybrid_empty) {
                    ++hybrid_this_round;
                    if (detailed_stats) {
        ...
        size_t round_bridge_committed = 0;
        size_t round_unaligned_bridge_paths = 0;
        ...
        std::unordered_set<const Segment*> minipoa_dirty_seen;
        minipoa_dirty_seen.reserve(65536);
        ...
        anchor_counts.reserve(candidate_slots.size());
        ...
        Realignment::MissingWindowBatchScheduler scheduler(
            std::move(remaining), footprints, anchor_counts);

        while (!scheduler.empty()) {
            const std::vector<size_t> batch = scheduler.nextBatch(
                kMinipoaBatchMaximumWindows,
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(effective_threads)
            ...
            for (size_t slot = 0; slot < batch.size(); ++slot) {
                ...
                round_unaligned_bridge_paths +=
                    prepared_slots[slot]->unaligned_bridge_paths;
                if (candidate.full_participant_bridge) {
                    ++round_bridge_committed;
                    ++bridge_pattern_counts[
                        candidate.participant_pattern];
                    if (candidate_slots[candidate_index]
                            .extended_bridge) {
                        ++bridge_extended_committed_total;
                        const auto reference_path = std::find_if(
                            candidate.paths.begin(), candidate.paths.end(),
                            [&](const MissingWindowPathContext& path) {
                                return path.key.first == reference_species;
                            });
                        if (reference_path != candidate.paths.end()) {
                            bridge_extended_bases_total +=
                                reference_path->interval_length;
                        }
                    }
                } else if (candidate.hybrid_empty) {
                    ...
                    while (start && !start->isHead()) {
                        if (!start->isTail()) {
                            const bool first_visit =
                                minipoa_dirty_seen.insert(
                                    start.get()).second;
                            // A replacement may already have been visited by
                            // a higher-priority window to its right.  It is a
                            // full-participant barrier in that earlier walk,
                            // but as its own dirty start the scan must cross it
                            // once to cover the still-dirty region on its left.
                            if (!first_visit && !is_replacement) {
                                break;
                            }
                            if (first_visit) {
            ...
            scheduler.completeBatch(batch, batch_succeeded);
            maybeLogRealignmentProgress(
                reference_species, "minipoa-schedule",
                minipoa_rounds, minipoa_reference_visits,
                candidate_slots.size(), scheduler.pendingCandidates(),
                round_ordinary_committed + round_hybrid_committed +
                    round_bridge_committed,
                batch.size(), minipoa_started, minipoa_last_report);
        }

        conflict_deferred_total += scheduler.droppedConflicts();

        minipoa_invocations_total += round_invocations;
        reference_only_bypass_total += round_bypasses;
        unaligned_bridge_paths_total +=
            round_unaligned_bridge_paths;
        ...
        if (round_ordinary_committed + round_hybrid_committed +
                round_bridge_committed ==
            0) {
            spdlog::warn(
                "[species-mismatch-realign][minipoa-unified] round={} "
                "scanned_windows={} ordinary_candidates={} "
                "hybrid_candidates={} bridge_candidates={} batches={} "
                "minipoa_calls={} reference_only_bypass={} failed={} "
                "selected=0 status=stalled",
                minipoa_rounds, scanned_this_round,
                ordinary_this_round, hybrid_this_round,
                bridge_this_round, round_batches, round_invocations,
                round_bypasses, round_failures);
            break;
        }
        ...
        bridge_committed_total += round_bridge_committed;
        ...
        spdlog::debug(
            ...
            "bridge_committed={} unaligned_bridge_paths={} "
            "replaced_old_blocks={} blocks_before={} blocks_after={} "
            "dirty_starts={} wall_seconds={:.3f} status=committed",
            minipoa_rounds, scanned_this_round,
            ordinary_this_round, hybrid_this_round, bridge_this_round,
            round_batches, round_invocations, round_bypasses,
            round_failures, round_ordinary_committed,
            round_hybrid_committed, round_bridge_committed,
            round_unaligned_bridge_paths, round_replaced_old_blocks,
            round_blocks_before, round_blocks_after,
            minipoa_dirty_starts.size(),
            round_prepare_seconds);
    }

    spdlog::info(
        "[species-mismatch-realign][phase] reference={} phase=minipoa "
        "status=done rounds={} visited={} candidates={} committed={} "
        "calls={} wall_seconds={:.3f}",
        reference_species, minipoa_rounds, minipoa_reference_visits,
        ...
        std::chrono::duration<double>(
            std::chrono::steady_clock::now() - minipoa_started).count());

    {
        spdlog::info(
            "[species-mismatch-realign][phase] reference={} "
            "phase=final-audit status=start affected_paths={}",
            reference_species, affected_paths.size());
        const auto final_audit_started = std::chrono::steady_clock::now();
        AllocationPhaseScope audit_phase("final-pool-audit");
        if (!rebuildSamplingAndAuditPaths(
                *this, affected_paths, pool_editor)) {
            throw std::runtime_error(
        ...
        spdlog::info(
            "[species-mismatch-realign][phase] reference={} "
            "phase=final-audit status=done blocks={} wall_seconds={:.3f}",
            reference_species, pool_editor.activeCount(),
            std::chrono::duration<double>(
                std::chrono::steady_clock::now() - final_audit_started)
                .count());
    }
    ...
    spdlog::debug(
        ...
        "hybrid_candidates={} bridge_candidates={} minipoa_calls={} "
        "reference_only_bypass={} reference_empty_bypass={} "
        "preparation_failures={} "
        "fallback_batches={} conflict_deferred={} "
        "ordinary_committed={} hybrid_committed={} bridge_committed={} "
        "unaligned_bridge_paths={} "
        ...
        ordinary_candidates_total, hybrid_candidates_total,
        bridge_candidates_total, minipoa_invocations_total,
        ...
        hybrid_committed_total, bridge_committed_total,
        unaligned_bridge_paths_total, minipoa_scan_seconds,
    ...
    spdlog::info(
        ...
        "total_seconds={:.3f} bridge_extended_scanned={} "
        "bridge_extended_candidates={} bridge_extended_committed={} "
        "bridge_extended_rejected={} bridge_extended_bases={}",
        reference_species,
        ...
        std::chrono::duration<double>(
            std::chrono::steady_clock::now() - realignment_started).count(),
        bridge_extended_scanned_total,
        bridge_extended_candidates_total,
        bridge_extended_committed_total,
        bridge_extended_scanned_total -
    ...
    for (const auto& [pattern, count] : bridge_pattern_counts) {
        spdlog::info(
            "[species-mismatch-realign][gap-bridge] "
            "pattern={} committed={}",
            pattern, count);
    }
    ...
    spdlog::debug(
        ...
        "hybrid_minipoa_windows={} gap_bridge_windows={} "
        "minipoa_windows={} replaced_old_blocks={} "
        "new_full_species_blocks={}",
        reference_species, total_replaced_windows,
        zero_gap_replaced_windows, ordinary_committed_total,
        hybrid_committed_total, bridge_committed_total,
        minipoa_committed_total, total_replaced_old_blocks,
        total_replaced_windows);
