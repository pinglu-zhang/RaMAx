...
namespace hal {
    ...
    template <typename T> inline T Hdf5ExternalArray::getValue(hsize_t index, hsize_t offset) const {
    ...
    template <typename T> inline void Hdf5ExternalArray::setValue(hsize_t index, hsize_t offset, T val) {
        assert(offset + sizeof(T) <= _dataSize);
